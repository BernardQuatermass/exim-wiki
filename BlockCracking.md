...
Each authenticator  must contain `server_set_id` line. After the `begin authenticators` line the paragraph with PLAIN must contain `server_set_id = $auth2`, the paragraph with LOGIN must contain `server_set_id = $auth1`.
